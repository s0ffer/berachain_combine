...
class Berachain(Base):
    ...
    async def claim_faucet(self) -> None:
        """
            Claims tokens from the Berachain faucet.

            This function interacts with the Berachain faucet API, solves the CAPTCHA using
        ...
        logger.info(f'Starting faucet claim | {self.client.account.address}')
        ...
        async with httpx.AsyncClient(proxy=self.client.proxy) as client:
            ...
            params: dict[str, str] = {'address': f'{self.client.account.address}'}

            await client.options('https://bartiofaucet.berachain.com/api/claim', params=params, headers=headers)
            headers = {
                'accept': '*/*',
                'accept-language': 'ru-RU,ru;q=0.9,en-US;q=0.8,en;q=0.7',
            ...
            params = {'address': f'{self.client.account.address}'}
            data_post = {"address": f"{self.client.account.address}"}
            response = await client.post('https://bartiofaucet.berachain.com/api/claim',
                                         params=params,
                                         headers=headers,
                                         json=data_post)
        ...
        if type(result) is int:
            logger.error(f'Insufficient ETH balance, need 0.001 | {self.client.account.address}')
        else:
            msg = result.get("msg", "")
            if 'Added' in msg:
                logger.success(f'Claimed faucet | {self.client.account.address} | '
                               f'{round((await self.client.wallet.balance()).Ether, 2)} BERA')
            elif 'exceeded' in msg:
                cut_msg = (msg.split('wait '))[1]
                claim_time = (cut_msg.split(' before'))[0]
                logger.error(f'Rate limit exceeded, wait {claim_time} | {self.client.account.address} | '
                             f'{round((await self.client.wallet.balance()).Ether, 2)} BERA')
            else:
                logger.error(msg)
