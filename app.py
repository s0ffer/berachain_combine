...
from tasks.berachain import Berachain
...
async def process_wallet(private_key: str, proxy: str, api_key: str, semaphore: Semaphore, selected_function: str):
    ...
    async with semaphore:
        ...
        berachain = Berachain(client=client,
                              api_key=api_key,
                              proxy_info=proxy_dict)

        if selected_function == 'claim_faucet':
            await berachain.claim_faucet()
        elif selected_function == 'get_bera_balance':
            await berachain.get_bera_balance()
